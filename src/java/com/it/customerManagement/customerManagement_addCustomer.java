...
//                                check customer name
                                UmCustomer checkName = new com.it.customerManagementModel.CMS_UM_Customer().checkCustomerName(ses, fullName);
                                if (checkName != null) {
                                    resultMap.put("result", "0");
                                    resultMap.put("displayMessage", "Name Already Exist");
                                } else {
//                                    check mobile number
                                    UmCustomer checkNumber = new com.it.customerManagementModel.CMS_UM_Customer().checkCustomerPhoneNumber(ses, mobile);
                                    if (checkNumber != null) {
                                        resultMap.put("result", "0");
                                        resultMap.put("displayMessage", "Mobile Number Already Exist");
                                    } else {
//                            save new customer
                                        UmCustomer addNewCustomer = new com.it.customerManagementModel.CMS_UM_Customer().saveNewCustomer(ses, fullName, address, STATIC_DATA_MODEL.PMACTIVE, mobile, isverify, otpCode, nic, email, addedDate, updatedDate, selectedBranch, activeUser.getId(), activeUser.getId());
                                        if (addNewCustomer != null) {
                                            ...
                                            SmSessionActivity sessionActivityToAddNewCustomer = new com.it.userManagementModel.UMS_UM_Session_Activity().saveSessionActivityNew(ses, saveNewCustomerSessionActivityContent, new Date(), STATIC_DATA_MODEL.INSERT, STATIC_DATA_MODEL.CUSTOMERMANAGEMENT, addNewCustomer.getId(), activeSession, activeUser);
