 ...
 * @author JOY
 */
@WebServlet(name = "customerManagement_sendOTP", urlPatterns = {"/customerManagement_sendOTP"})
public class customerManagement_sendOTP extends HttpServlet {
...
//                    System.out.println("mobile = " + mobile);
                    if (mobile.isEmpty()) {
                        ...
                        resultMap.put("displayMessage", "Enter Mobile NumberB");
                    } else {
//                        int hh = Integer.parseInt(mobile);
//                        generate OTP code
                        String otp = new DecimalFormat("000000").format(new Random().nextInt(999999));
                        
                        String msg = "Use "+otp +" for account verification. Vision Care Help Desk";
//                        System.out.println(otp);
//                        char opt[] = STATIC_DATA_MODEL.OTP(4);
//                        System.out.println("opt = " + opt[0123]);
                        String smsStatus = SMS_MODEL.Send_SMS(mobile,msg);
//                        System.out.println("sms status = " + smsStatus);
                        if (smsStatus.equals("200")) {
                            HttpSession httpSession = request.getSession();
                            httpSession.setAttribute("custOTP", otp);
                            resultMap.put("result", "1");
                            resultMap.put("displayMessage", "OTP Send Done");
                        } else {
                            resultMap.put("result", "0");
