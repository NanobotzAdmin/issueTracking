...
//                            check user role has interface component esixts
                            PmUserRoleHasInterfaceComponent checkUserRoleHasCompId = new com.it.privilegeManagementModel.PMS_PM_User_Role_Has_Interface_Component().checlUserRoleHasComponent(session, selectedUserRole.getId(), selectedComponent.getId());
                            if (checkUserRoleHasCompId == null) {
                                PmUserRoleHasInterfaceComponent addComponentToUserRole = new com.it.privilegeManagementModel.PMS_PM_User_Role_Has_Interface_Component().addUserRoleHasInterfaceComponent(session, selectedUserRole.getId(), selectedComponent.getId(), addedDate, updatedDate, STATIC_DATA_MODEL.PMACTIVE, activeUser, activeUser);
...
//                                                    add interfaces to users in that user role   
                                            UmUserHasInterfaceComponent addSelectedUserToComponent = new com.it.privilegeManagementModel.PMS_PM_User_Has_Interface_Component().addUserHasInterfaceComponent(session, usersByURole.getId(), selectedComponent.getId(), updatedDate, updatedDate, STATIC_DATA_MODEL.PMACTIVE, activeUser, activeUser);
//                                        CHECK user has interface topic exist
                                            UmUserHasInterfaceTopic checkUserHasInterfaceTopicExist = new com.it.privilegeManagementModel.PMS_PM_User_Has_Interface_Topic().getAllInterfacetopicAndUser(session, usersByURole.getId(), selectedTopic.getId());
...
//                                    deleteUserRole has Interfaces
                            PmUserRoleHasInterfaceComponent deleteComponent = new com.it.privilegeManagementModel.PMS_PM_User_Role_Has_Interface_Component().removeUserRoleHasComponent(session, selectedUserRole.getId(), selectedComponent.getId());
                            if (deleteComponent != null) {
//                                deleteUserHasComponents
                                int checkComponentDeleted = new com.it.privilegeManagementModel.PMS_PM_User_Has_Interface_Component().deleteUserHasInterfaceComponentByUserRoleId(session, selectedUserRole.getId(), selectedComponent.getId());
                                if (checkComponentDeleted > 0) {
                                    ...
                                    if (!loadAllUsersBYUserRole2.isEmpty()) {
                                        for (UmUser umUser2 : loadAllUsersBYUserRole2) {
                                            ...
                                            SmSessionActivity sessionActivityToComponentRemoveFromUsersInUserRole = new com.it.userManagementModel.UMS_UM_Session_Activity().saveSessionActivity(session, removeintCompoToUserRoleContent, new Date(), STATIC_DATA_MODEL.DELETE, STATIC_DATA_MODEL.USERROLEHASICOMPONENTREMOVE, activeSession, activeUser);
                                        }
                                    }
...
//                                        ////System.out.println("delete records size =" + deleteRecords);
                                        if (deleteRecords > 0) {
                                            List<UmUser> loadAllUsersBYUserRole3 = new com.it.userManagementModel.UMS_UM_User().searchUsersByStatusAndUserRole(session, selectedUserRole.getId(), STATIC_DATA_MODEL.ACTIVE);
