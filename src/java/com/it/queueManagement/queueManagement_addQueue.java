...
//                                check queue name exists
                            QmQueue checkName = new com.it.queueManagementModel.QMS_QM_Queue().checkNameExists(ses, queueName);
                            if (checkName != null) {
                                resultMap.put("result", "0");
                                resultMap.put("displayMessage", "Queue Exists.");
                            } else {
//                           save queue details
                                QmQueue addNewQueue = new com.it.queueManagementModel.QMS_QM_Queue().saveQueue(ses, queueName, queueDescription, null, queueicon, queueFontColor, STATIC_DATA_MODEL.PMACTIVE, addedDate, addedDate, activeUser.getId(), activeUser.getId());
                                if (addNewQueue != null) {
                                    ...
                                    SmSessionActivity sessionActivityToAddQueue = new com.it.userManagementModel.UMS_UM_Session_Activity().saveSessionActivity(ses, saveQueueSessionActivityContent, new Date(), STATIC_DATA_MODEL.INSERT, STATIC_DATA_MODEL.QUEUESAVE, activeSession, activeUser);
