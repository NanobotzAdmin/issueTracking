...
//                      add user to Ticket
                                TmTicketsHasUmUser addUsersToTicket = new com.it.ticketManagementModel.TMS_TM_Tickets_Has_Um_User().addTicketHasUsers(ses, selectedTicket.getId(), selectedUser.getId(), STATIC_DATA_MODEL.PMACTIVE, updatedDate, updatedDate, activeUser.getId(), activeUser.getId());
                                String saveTicketHasUserSessionActivityContent = selectedUser.getFirstName() + " User has been Added To " + selectedTicket.getTicketName() + " Issue by " + activeUser.getFirstName() + " on " + addedDate;
                                SmSessionActivity sessionActivityToAddUserToTicket = new com.it.userManagementModel.UMS_UM_Session_Activity().saveSessionActivityNew(ses, saveTicketHasUserSessionActivityContent, new Date(), STATIC_DATA_MODEL.INSERT, STATIC_DATA_MODEL.ISSUESMANAGEMENT, selectedTicket.getId(), activeSession, activeUser);
                                if (selectedUser.getMobileNumber() != null) {
                                    String smsCnt = activeUser.getFirstName() + " " + activeUser.getLastName() + " has given you a new Issue ";
                                    String smsStatus = SMS_MODEL.Send_SMS(selectedUser.getMobileNumber(), smsCnt);
                                }
                                tr.commit();
