 ...
 */
package com.it.locationManagement;
...
import com.it.db.LmLocationsHasUmUser;
...
//                            check location name exists
                            LmLocations checkName = new com.it.locationManagementModel.LMS_LM_Locations().checkNameExists(ses, locationName);
                            if (checkName != null) {
                                resultMap.put("result", "0");
                                resultMap.put("displayMessage", "Branch Name Exists.");
                            } else {
//                            check address exists
                                LmLocations checkAddress = new com.it.locationManagementModel.LMS_LM_Locations().checkAddressExists(ses, address);
                                if (checkAddress != null) {
                                    resultMap.put("result", "0");
                                    resultMap.put("displayMessage", "Branch Address Exists.");
                                } else {
//                            save location details
                                    LmLocations addNewLocation = new com.it.locationManagementModel.LMS_LM_Locations().saveLocation(ses, locationName, address, mobile, landnumber, STATIC_DATA_MODEL.ISACTIVE, updatedDate, updatedDate, selectedBranchManager, activeUser.getId(), activeUser.getId());
                                    if (addNewLocation != null) {
                                        ...
                                        SmSessionActivity sessionActivityToAddLocation = new com.it.userManagementModel.UMS_UM_Session_Activity().saveSessionActivity(ses, saveLocationSessionActivityContent, new Date(), STATIC_DATA_MODEL.INSERT, STATIC_DATA_MODEL.LOCATIONSAVE, activeSession, activeUser);
                                        if (sessionActivityToAddLocation != null) {
//                                    save location history
                                            String locationHistoryDescription = addNewLocation.getLocationName() + " Location Saved";
                                            LmLocationHistory addLocationHistory = new com.it.locationManagementModel.LMS_LM_Location_History().saveLocationHistory(ses, locationHistoryDescription, updatedDate, updatedDate, addNewLocation, activeUser.getId(), activeUser.getId());
...
//                                                    add user to location
                                                            LmLocationsHasUmUser addUsersToLocation = new com.it.locationManagementModel.LMS_LM_Locations_Has_Um_User().addLocationHasUsers(ses, addNewLocation.getId(), selecUser.getId(), STATIC_DATA_MODEL.ISACTIVE, updatedDate, updatedDate, activeUser.getId(), activeUser.getId());
                                                        }
                                                    }
                                                }
                                                tr.commit();
