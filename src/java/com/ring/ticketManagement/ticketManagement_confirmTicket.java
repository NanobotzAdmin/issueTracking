...
//                        get ticket has reply
                        ...
                        String TicketConfirmedSessionActivityContent = selectedTicket.getTicketName() + " Ticket has been Confirmed by " + activeUser.getFirstName() + " on " + updatedDate;
                        SmSessionActivity sessionActivityToTicketConfirmed = new com.ring.userManagementModel.UMS_UM_Session_Activity().saveSessionActivityNew(ses, TicketConfirmedSessionActivityContent, new Date(), STATIC_DATA_MODEL.UPDATE, STATIC_DATA_MODEL.TICKETMANAGEMENT, selectedTicket.getId(), activeSession, activeUser);
                            ...
                            for (TmTicketsHasUmUser ticketHasUser : getTicketHasUsers) {
                                if (ticketHasUser.getUmUser().getEmailAddress() != null) {
                                    ...
                                    String emailContent = "<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">\n"
                                            ...
                                            + "                                                                                                                                </body>\n"
                                            + "                                                                                                                                </html>";
                                    String emailSubject = "Ticket Confirmed  | " + selectedTicket.getQmQueue().getQueueName() + " | " + selectedTicket.getTid();

                                    EMAIL_MODEL.Sendmail(ticketHasUser.getUmUser().getEmailAddress(), emailSubject, emailContent);
                                }
                            }
                        }
                        //                        save ticket comfirm reply 
                        TmTicketReply saveComfirmReply = new com.ring.ticketManagementModel.TMS_TM_Ticket_Reply().saveTicketReply(ses, "Ticket has been <span class=\"badge bg-black\"> Confirm </span>", STATIC_DATA_MODEL.PMACTIVE, 0.0, updatedDate, updatedDate, selectedTicket, activeUser.getId(), activeUser.getId());
                        tr.commit();
                        resultMap.put("result", "1");
                        resultMap.put("displayMessage", "Ticket Confirmed Done");
                    } else {
