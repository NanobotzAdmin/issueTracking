 ...
 * @author JOY
 */
@WebServlet(name = "ticketManagement_completeTicket", urlPatterns = {"/ticketManagement_completeTicket"})
public class ticketManagement_completeTicket extends HttpServlet {
...
//                        Date date1 = new Date(selectedTicket.getCreatedAt());
                        ...
                        selectedTicket.setStatus(STATIC_DATA_MODEL.TICKETCOMPLETED);
                        selectedTicket.setUpdatedAt(updatedDate);
                        selectedTicket.setUpdatedBy(activeUser.getId());
                        selectedTicket.setCompletedBy(activeUser.getId());
                        selectedTicket.setTimeToComplete(difference_In_Time);
                        selectedTicket.setCompletedAt(updatedDate);
                        ses.update(selectedTicket);
                        String TicketCompletedSessionActivityContent = selectedTicket.getTicketName() + " Ticket has been Completed by " + activeUser.getFirstName() + " on " + updatedDate;
                        ...
                        if (!getTicketHasUsers.isEmpty()) {
                            for (TmTicketsHasUmUser ticketHasUser : getTicketHasUsers) {
                                if (ticketHasUser.getUmUser().getEmailAddress() != null) {
                                    ...
                                    String emailSubject = "Ticket Completed | " + selectedTicket.getQmQueue().getQueueName() + " | " + selectedTicket.getTid();
                                    EMAIL_MODEL.Sendmail(ticketHasUser.getUmUser().getEmailAddress(), emailSubject, emailContent);
                                }
                            }
                        }
//                        save ticket complete reply 
                        TmTicketReply saveCompleteReply = new com.ring.ticketManagementModel.TMS_TM_Ticket_Reply().saveTicketReply(ses, "Ticket has been <span class=\"badge bg-primary\"> Complete </span>", STATIC_DATA_MODEL.PMACTIVE, 0.0, updatedDate, updatedDate, selectedTicket, activeUser.getId(), activeUser.getId());
                        tr.commit();
                        resultMap.put("result", "1");
                        resultMap.put("displayMessage", "Ticket Completed Done");
                    } else {
