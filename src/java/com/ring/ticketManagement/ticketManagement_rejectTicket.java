 ...
 * @author JOY
 */
@WebServlet(name = "ticketManagement_rejectTicket", urlPatterns = {"/ticketManagement_rejectTicket"})
public class ticketManagement_rejectTicket extends HttpServlet {
...
//                        Date date1 = new Date(selectedTicket.getCreatedAt());
                        ...
                        selectedTicket.setCompletedBy(null);
                        selectedTicket.setCompletedAt(null);
                        selectedTicket.setTimeToComplete(null);
                        selectedTicket.setCompletedAt(updatedDate);
                        ses.update(selectedTicket);
                        String TicketCompletedSessionActivityContent = selectedTicket.getTicketName() + " Ticket has been Rejected by " + activeUser.getFirstName() + " on " + updatedDate;
                        SmSessionActivity sessionActivityToTicketCompleted = new com.ring.userManagementModel.UMS_UM_Session_Activity().saveSessionActivityNew(ses, TicketCompletedSessionActivityContent, new Date(), STATIC_DATA_MODEL.UPDATE, STATIC_DATA_MODEL.TICKETMANAGEMENT, selectedTicket.getId(), activeSession, activeUser);
                        ...
                        if (!getTicketHasUsers.isEmpty()) {
                            for (TmTicketsHasUmUser ticketHasUser : getTicketHasUsers) {
                                if (ticketHasUser.getUmUser().getEmailAddress() != null) {
                                    ...
                                    String emailSubject = "Ticket Rejected | " + selectedTicket.getQmQueue().getQueueName() + " | " + selectedTicket.getTid();
                                    EMAIL_MODEL.Sendmail(ticketHasUser.getUmUser().getEmailAddress(), emailSubject, emailContent);
                                }
                            }
                        }
//                        save ticket reject reply 
                        TmTicketReply saveRejectReply = new com.ring.ticketManagementModel.TMS_TM_Ticket_Reply().saveTicketReply(ses, "Ticket has been <span class=\"badge bg-danger\"> Rejected </span>", STATIC_DATA_MODEL.PMACTIVE, 0.0, updatedDate, updatedDate, selectedTicket, activeUser.getId(), activeUser.getId());
                        tr.commit();
                        resultMap.put("result", "1");
                        resultMap.put("displayMessage", "Ticket Reject Done");
                    } else {
