 ...
 * @author JOY
 */
@WebServlet(name = "queueManagement_removeUsersFromQueueUpdate", urlPatterns = {"/queueManagement_removeUsersFromQueueUpdate"})
public class queueManagement_removeUsersFromQueueUpdate extends HttpServlet {
...
//                        selected User 
 ...
 //                    check queue has user already exists
                            QmQueueHasUser checkUserExists = new com.ring.queueManagementModel.QMS_QM_Queue_Has_User().getUsersByQueueId(ses, selectedQueue.getId(), selectedUser.getId());
//                            delete user
                            ses.delete(checkUserExists);
                            String removeUserFromQueueSessionActivityContent = selectedUser.getFirstName() + " User has been Removed From " + selectedQueue.getQueueName()+ " Queue by " + activeUser.getFirstName() + " on " + addedDate;
                            SmSessionActivity sessionActivityToRemoveUserFromQueue = new com.ring.userManagementModel.UMS_UM_Session_Activity().saveSessionActivityNew(ses, removeUserFromQueueSessionActivityContent, new Date(), STATIC_DATA_MODEL.DELETE, STATIC_DATA_MODEL.QUEUEMANAGEMENT, selectedQueue.getId(), activeSession, activeUser);
                            tr.commit();
