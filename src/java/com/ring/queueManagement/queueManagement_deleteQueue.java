 ...
 * @author JOY
 */
@WebServlet(name = "queueManagement_deleteQueue", urlPatterns = {"/queueManagement_deleteQueue"})
public class queueManagement_deleteQueue extends HttpServlet {
...
//                                    delete queue
                            ses.delete(selectedQueue);
                            //Get Active session object
                            SmSession activeSession = (SmSession) request.getSession().getAttribute("logUserHttpSession");
                            //Save session activity
                            String deleteQueueStatusSessionActivityContent = selectedQueue.getQueueName() + " Queue has been Deleted by " + activeUser.getFirstName() + " on " + updatedDate;
                            SmSessionActivity sessionActivityTodeleteQueue = new com.ring.userManagementModel.UMS_UM_Session_Activity().saveSessionActivityNew(ses, deleteQueueStatusSessionActivityContent, new Date(), STATIC_DATA_MODEL.DELETE, STATIC_DATA_MODEL.QUEUEMANAGEMENT, selectedQueue.getId(), activeSession, activeUser);
                            if (sessionActivityTodeleteQueue != null) {
                                tr.commit();
                                resultMap.put("result", "1");
                                resultMap.put("displayMessage", "Delete Success");
                            } else {
                                resultMap.put("result", "0");
