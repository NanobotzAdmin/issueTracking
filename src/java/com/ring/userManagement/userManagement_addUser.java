...
//                                check user nic exists
                                UmUser checkNicExists = new com.ring.userManagementModel.UMS_UM_User().checkNic(ses, nic);
                                if (checkNicExists != null) {
                                    resultMap.put("result", "0");
                                    resultMap.put("displayMessage", "NIC Already Exists");
                                } else {
//                                    check email
                                    UmUser checkEmailExists = new com.ring.userManagementModel.UMS_UM_User().checkEmail(ses, email);
                                    if (checkEmailExists != null) {
                                        resultMap.put("result", "0");
                                        resultMap.put("displayMessage", "Email Already Exists");
                                    } else {
//                                        check employee Id
                                        UmUser checkEmployeeIdExists = new com.ring.userManagementModel.UMS_UM_User().checkEmail(ses, email);
                                        if (checkEmployeeIdExists != null) {
                                            resultMap.put("result", "0");
                                            resultMap.put("displayMessage", "Employee Id Already Exists");
                                        } else {
//                            save new user
                                            UmUser addNewUser = new com.ring.userManagementModel.UMS_UM_User().saveNewUser(ses, fName, lName, STATIC_DATA_MODEL.PMACTIVE, empId, pass, nic, dob, mobile, email, gndr, maritalStatus, empId, addedDate, updatedDate, selectedUserRole, activeUser.getId(), activeUser.getId());
                                            if (addNewUser != null) {
                                                ...
                                                SmSessionActivity sessionActivityToAddNewUser = new com.ring.userManagementModel.UMS_UM_Session_Activity().saveSessionActivityNew(ses, saveNewUserSessionActivityContent, new Date(), STATIC_DATA_MODEL.INSERT, STATIC_DATA_MODEL.USERMANAGEMENT, addNewUser.getId(), activeSession, activeUser);
                                                if (sessionActivityToAddNewUser != null) {
                                                    ...
                                                    if (!loadUserRoleHasAllComponent.isEmpty()) {
                                                        for (PmUserRoleHasInterfaceComponent compoByURole : loadUserRoleHasAllComponent) {
                                                            ...
                                                            UmUserHasInterfaceComponent addSelectedUserToComponent = new com.ring.privilegeManagementModel.PMS_PM_User_Has_Interface_Component().addUserHasInterfaceComponent(ses, addNewUser.getId(), compoByURole.getPmInterfaceComponent().getId(), addedDate, addedDate, STATIC_DATA_MODEL.PMACTIVE, activeUser, activeUser);
                                                            ...
                                                            SmSessionActivity sessionActivityToComponentAssignUser = new com.ring.userManagementModel.UMS_UM_Session_Activity().saveSessionActivityNew(ses, saveintCompoToUserContent, new Date(), STATIC_DATA_MODEL.INSERT, STATIC_DATA_MODEL.USERHASICOMPONENTSAVE, addNewUser.getId(), activeSession, activeUser);
                                                            //                              load user has interface topic exist
                                                            UmUserHasInterfaceTopic checkUserHasInterfaceTopicExist = new com.ring.privilegeManagementModel.PMS_PM_User_Has_Interface_Topic().getAllInterfacetopicAndUser(ses, addNewUser.getId(), selectedTopic.getId());
                                                            if (checkUserHasInterfaceTopicExist == null) {
//                                            add user has interface component
                                                                xx++;
                                                                UmUserHasInterfaceTopic addUserHasInterfaceHas = new com.ring.privilegeManagementModel.PMS_PM_User_Has_Interface_Topic().saveInterfacetopicHasUser(ses, addNewUser.getId(), selectedTopic.getId(), STATIC_DATA_MODEL.PMACTIVE, xx, addedDate, addedDate, activeUser, activeUser);
                                                                ...
                                                                SmSessionActivity saveInterfaceTopicHasUserSessionActivityt = new com.ring.userManagementModel.UMS_UM_Session_Activity().saveSessionActivityNew(ses, saveinterfaceTopicHasUserSessionActivity, new Date(), STATIC_DATA_MODEL.INSERT, STATIC_DATA_MODEL.USERHASITOPICSAVE, addNewUser.getId(), activeSession, activeUser);
                                                            }
                                                        }
                                                    }
                                                    tr.commit();
