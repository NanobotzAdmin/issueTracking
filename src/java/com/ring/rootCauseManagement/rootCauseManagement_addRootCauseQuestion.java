...
//                            save Root Cause Question details
                            RcasQuestion addNewRootCauseQuestion = new com.ring.rootCauseManagementModel.RCMS_Rcas_Question().saveRootCauseQuestionQueue(ses, rcasQuestion, STATIC_DATA_MODEL.PMACTIVE, updatedDate, updatedDate, activeUser.getId(), activeUser.getId());
                            if (addNewRootCauseQuestion != null) {
                                ...
                                SmSessionActivity sessionActivityToAddRCQ = new com.ring.userManagementModel.UMS_UM_Session_Activity().saveSessionActivityNew(ses, saveRCQSessionActivityContent, new Date(), STATIC_DATA_MODEL.INSERT, STATIC_DATA_MODEL.QUEUESAVE,addNewRootCauseQuestion.getId(),activeSession, activeUser);
...
//                                        add location has users
                                        ...
                                        if (r > 0) {
                                            for (int i = 0; i < r; ++i) {
                                                ...
                                                if (selecQueue != null) {
//                                                    add Queue to RCQ
                                                    QmQueueHasQuestion addQueuesToRCQ = new com.ring.rootCauseManagementModel.RCMS_QM_Queue_Has_Question().addRCQHasQueue(ses, selecQueue.getId(), addNewRootCauseQuestion.getId(), STATIC_DATA_MODEL.PMACTIVE, updatedDate, updatedDate, activeUser.getId(), activeUser.getId());
                                                }
                                            }
                                        }
                                        tr.commit();
